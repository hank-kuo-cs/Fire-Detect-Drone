...
def postprocess(frame, outs, orgFrame):
    ...
    for out in outs:
        for detection in out:
            ...
            if confidence > confThreshold:
                ...
                result.append([1, classes[classId], confidence, center_x, center_y, width, height, left, top])
    ...
    width = box[2]
    height = box[3]
    drawPred(frame, classIds[i], confidences[i], left, top, left + width, top + height, orgFrame)
...
def drawPred(frame, classId, conf, left, top, right, bottom, orgFrame):
    label = '%.2f' % conf
...
def detect_fire(image):
    ...
    result = postprocess(image, outs, orgFrame)
    t, _ = net.getPerfProfile()

    if result['is_fire']:
        result['image'] = image.astype(np.uint8)
